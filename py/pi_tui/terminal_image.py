"""Terminal image rendering (Kitty graphics protocol, iTerm2).

Handles detection of terminal image capabilities and encoding images
for display using Kitty and iTerm2 inline image protocols.
"""
...
_cell_dimensions: CellDimensions = CellDimensions(width_px=9, height_px=18)
...
def detect_capabilities() -> TerminalCapabilities:
    """Check environment variables to detect terminal image capabilities."""
...
def get_capabilities() -> TerminalCapabilities:
    """Return cached terminal capabilities, detecting on first call."""
...
def reset_capabilities_cache() -> None:
    """Clear the cached capabilities so next call re-detects."""
    global _cached_capabilities
    _cached_capabilities = None


def is_image_line(line: str) -> bool:
    """Check if a line contains Kitty or iTerm2 image escape sequences."""
...
def allocate_image_id() -> int:
    ...
    Uses random IDs in [1, 0xffffffff] to avoid collisions between
    different module instances (e.g., main app vs extensions).
    """
...
def encode_kitty(
    ...
    *,
    ...
    Chunks base64 data at 4096 bytes per chunk.
    """
    chunk_size = 4096
...
def encode_iterm2(
    ...
    *,
    ...
    inline_val = 0 if inline is False else 1
    params: list[str] = [f"inline={inline_val}"]
    ...
    if name:
        name_b64 = base64.b64encode(name.encode()).decode("ascii")
...
def get_jpeg_dimensions(base64_data: str) -> ImageDimensions | None:
    ...
    try:
        ...
        while offset < len(data) - 9:
            ...
            # SOF0, SOF1, SOF2
            if 0xC0 <= marker <= 0xC2:
                height: int = struct.unpack(">H", data[offset + 5 : offset + 7])[0]
                width: int = struct.unpack(">H", data[offset + 7 : offset + 9])[0]
                return ImageDimensions(width_px=width, height_px=height)

            if offset + 3 >= len(data):
                return None
            length: int = struct.unpack(">H", data[offset + 2 : offset + 4])[0]
            if length < 2:
                return None
            offset += 2 + length
...
def get_gif_dimensions(base64_data: str) -> ImageDimensions | None:
    """Parse GIF header to extract image dimensions."""
    try:
        data = base64.b64decode(base64_data)
        if len(data) < 10:
            return None
        sig = data[:6].decode("ascii")
        if sig not in ("GIF87a", "GIF89a"):
            return None
        width: int = struct.unpack("<H", data[6:8])[0]
        height: int = struct.unpack("<H", data[8:10])[0]
        return ImageDimensions(width_px=width, height_px=height)
    except Exception:
        return None
...
def get_webp_dimensions(base64_data: str) -> ImageDimensions | None:
    """Parse WebP header to extract image dimensions (VP8, VP8L, VP8X)."""
    try:
        data = base64.b64decode(base64_data)
        if len(data) < 30:
            return None
        ...
        if chunk == "VP8 ":
            ...
            if len(data) < 25:
                return None
            bits: int = struct.unpack("<I", data[21:25])[0]
            width = (bits & 0x3FFF) + 1
...
def get_image_dimensions(base64_data: str, mime_type: str) -> ImageDimensions | None:
    """Dispatch to format-specific dimension parser based on MIME type."""
...
def render_image(
    ...
    Returns a RenderResult with the escape sequence, row count, and
    optional image ID, or None if no image protocol is available.
    """
    if options is None:
    ...
    if caps.images == "kitty":
        ...
        return RenderResult(sequence=sequence, rows=rows, image_id=options.image_id)
    ...
    if caps.images == "iterm2":
        sequence = encode_iterm2(
            ...
            width=max_width,
            height="auto",
            preserve_aspect_ratio=options.preserve_aspect_ratio if options.preserve_aspect_ratio is not None else True,
        )
        return RenderResult(sequence=sequence, rows=rows)
...
def image_fallback(
    ...
    """Generate a text fallback for an image that cannot be displayed inline."""
    parts: list[str] = []
    if filename:
