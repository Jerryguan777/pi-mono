    ...
    for i in range(start_index, end_index):
        ...
        if entry_type == "message":
            ...
            if msg is not None:
                ...
                if msg_role in ("bashExecution", "custom", "branchSummary", "compactionSummary", "user", "assistant"):
                    cut_points.append(i)
...
def _get_message_from_entry(entry: Any) -> AgentMessage | None:
    ...
    if entry_type == "branch_summary":
        ...
        return _make_summary_message("branchSummary", summary or "")  # type: ignore[no-any-return]

    if entry_type == "compaction":
        summary = entry.get("summary") if isinstance(entry, dict) else getattr(entry, "summary", "")
        return _make_summary_message("compactionSummary", summary or "")  # type: ignore[no-any-return]
