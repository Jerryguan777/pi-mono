...
from pi_agent.types import AgentMessage, AgentState, ThinkingLevel
...
from pi_coding_agent.core.model_resolver import ScopedModel, find_initial_model
...
async def create_agent_session(
    ...
    if options.tools is not None:
        from pi_coding_agent.core.tools import all_tools
    ...
    def _convert_to_llm_with_block_images(messages: list[AgentMessage]) -> list[Message]:
        ...
        if not settings_manager.get_block_images():
            return converted
        from pi_ai.types import ImageContent, TextContent, UserMessage
...
def _convert_thinking_budgets(settings_manager: SettingsManager) -> Any:
    """Convert settings thinking budgets to pi_ai ThinkingBudgets."""
    budgets = settings_manager.get_thinking_budgets()
    if budgets is None:
        return None
    from pi_ai.types import ThinkingBudgets
